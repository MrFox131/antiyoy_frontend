...
app.loader
    ...
    .add("up", "../src/img/up.png")
    .add("down", "../src/img/down.png")
    .load((loader, resources) => {
        const mainScene = new PIXI.Container();
        mainScene.position.set(window.innerWidth / 2, window.innerHeight / 2);
        mainScene.pivot.set(window.innerWidth / 2, window.innerHeight / 2);
        ...
        const buttonBlock = new PIXI.Container();
        buttonBlock.position.set(window.innerWidth / 2, window.innerHeight / 2);
        mainScene.addChild(buttonBlock);

        const up = new PIXI.Sprite(resources.up.texture);
        up.anchor.set(1, 0.5);
        up.scale.set(0.1);
        up.interactive = true;
        up.buttonMode = true;
        up.on("pointerdown", ()=>{
            mainScene.scale.x *= 1.25;
            mainScene.scale.y *= 1.25;
            console.log(mainScene.position);
        });
        buttonBlock.addChild(up);

        const down = new PIXI.Sprite(resources.down.texture);
        down.scale.set(0.1);
        down.anchor.set(0, 0.5);
