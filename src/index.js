...
app.loader
    ...
    .load((loader, resources) => {
        ...
        boardSprite.on('pointerover', () => {
            zoomPermission = true;
        });
        boardSprite.on('pointerout', () => {
        ...
        app.ticker.add((delta) => {
            if (keyboardState.UP || keyboardState.DOWN || keyboardState.LEFT || keyboardState.RIGHT) {
                boardSprite.x = Math.max(Math.min(0, boardSprite.x + (keyboardState.LEFT * 30 - keyboardState.RIGHT * 30) / initialBoardScale / boardSprite.scale.x * delta), -boardSprite.width + boardSprite.width / boardSprite.scale.x);
                boardSprite.y = Math.max(Math.min(0, boardSprite.y + (keyboardState.UP * 30 - keyboardState.DOWN * 30) / initialBoardScale / boardSprite.scale.x * delta), -boardSprite.height + boardSprite.height / boardSprite.scale.y);
            }
        });
    });